            ...
            <div className="space-y-6">
              ...
              {payrollTab === 'Employee Base Salaries' && (
                <div className="glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
                  ...
                  <DataTable columns={[
                    { key: 'name', label: 'Employee', render: (row) => <span className="font-extrabold text-slate-900">{row.workerName || 'Staff'}</span> },
                    { key: 'role', label: 'Position', render: (row) => <span className="text-slate-500">{row.position || 'Staff'}</span> },
                    { key: 'rate', label: 'Hourly/Salaried', render: (row) => <span className="font-mono text-xs">{(() => { let v = row.rateType || row.rate; if (typeof v === 'object' && v !== null) { v = v.rateType || v.name || JSON.stringify(v); } return typeof v === 'object' ? JSON.stringify(v) : String(v || ''); })()}</span> },
                    { key: 'salary', label: 'Salary Net', render: (row) => <span className="font-mono font-bold text-slate-600">{row.amount || (row.net ? `$${row.net.toLocaleString()}` : '')}</span> },
                    { key: 'status', label: 'Status', render: (row) => <StatusBadge status={row.status} /> },
                    {
                      key: 'actions', label: 'Disburse', render: (row) => (
                        row.status === 'Pending' ? (
              ...
              <div className="lg:col-span-7 glass rounded-2xl p-5 border border-slate-200 text-left space-y-4">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-[#2E2E2E]/35">
